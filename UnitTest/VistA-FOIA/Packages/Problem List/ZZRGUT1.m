ZZRGUT1 ;RGI/VSL - Unit Tests - Problem List ;4/24/12
...
ASSUSR ; Test Assign list to users (ASSGNUSR)
 K:$D(RETURN) RETURN
 D ASSUSR^GMPLAPI6(.RETURN,"A","")
 D CHKEQ^XTMUNIT("INVPARAM",$P(RETURN(0),U,1),"INVPARAM GMPLLST expected")
 ;
 K:$D(RETURN) RETURN
 D ASSUSR^GMPLAPI6(.RETURN,+NEWLST+1,"")
 D CHKEQ^XTMUNIT("LISTNFND",$P(RETURN(0),U,1),"LISTNFND expected")
 ;
 K:$D(RETURN) RETURN
 D ASSUSR^GMPLAPI6(.RETURN,+NEWLST,"^USER^")
 D CHKEQ^XTMUNIT("INVPARAM",$P(RETURN(0),U,1),"INVPARAM USER expected")
 ;
 K:$D(RETURN) RETURN
 D ASSUSR^GMPLAPI6(.RETURN,+NEWLST,$P(^VA(200,0),U,3)+1)
...
REMUSR ; Test Remove list from users
 K:$D(RETURN) RETURN
 D REMUSR^GMPLAPI6(.RETURN,"A",USER)
 D CHKEQ^XTMUNIT("INVPARAM",$P(RETURN(0),U,1),"INVPARAM GMPLLST expected")
 ;
 K:$D(RETURN) RETURN
 D REMUSR^GMPLAPI6(.RETURN,+NEWLST+1,USER)
 D CHKEQ^XTMUNIT("LISTNFND",$P(RETURN(0),U,1),"LISTNFND expected")
 ;
 K:$D(RETURN) RETURN
 D REMUSR^GMPLAPI6(.RETURN,+NEWLST,"^USER^")
 D CHKEQ^XTMUNIT("INVPARAM",$P(RETURN(0),U,1),"INVPARAM GMPLUSER expected")
 ;
 K:$D(RETURN) RETURN
 D REMUSR^GMPLAPI6(.RETURN,+NEWLST,$P(^VA(200,0),U,3)+1)
 D CHKEQ^XTMUNIT("PROVNFND",$P(RETURN(0),U,1),"PROVNFND expected")
 ;
 K:$D(RETURN) RETURN
 D REMUSR^GMPLAPI6(.RETURN,+NEWLST,USER)
...
DEULST ;
 ...
 ;
 K:$D(RETURN) RETURN
 D DELLST^GMPLAPI1(.RETURN,+NEWLST+1)
 D CHKEQ^XTMUNIT("LISTNFND",$P(RETURN(0),U,1),"LISTNFND expected")
 ;
 K:$D(RETURN) RETURN
 D DELLST^GMPLAPI1(.RETURN,+NEWLST)
...
ADDCAT ;
 ...
 ;
 K:$D(RETURN) RETURN
 D SAVLST^GMPLAPI1(.RETURN,+NEWLST+1,.TARGET)
 D CHKEQ^XTMUNIT("LISTNFND",$P(RETURN(0),U,1),"LISTNFND expected")
 ;
 K:$D(RETURN) RETURN
 D SAVLST^GMPLAPI1(.RETURN,+NEWLST,.TARGET)
 D CHKTF^XTMUNIT($D(RETURN(0))=0,"No error expected")
...
REMCAT ;
 ...
 D CHKTF^XTMUNIT($D(^GMPL(125.1,"B",+NEWCAT))=0,"INCORRECT CROSS-REFERENCE")
...
ADDPRB ;
 ...
 ;
 K:$D(RETURN) RETURN
 D SAVGRP^GMPLAPI1(.RETURN,+NEWCAT+1,.TARGET)
 D CHKEQ^XTMUNIT("CTGNFND",$P(RETURN(0),U,1),"LISTNFND expected")
 ;
 K:$D(RETURN) RETURN
 D SAVGRP^GMPLAPI1(.RETURN,+NEWCAT,.TARGET)
 D CHKTF^XTMUNIT($D(RETURN(0))=0,"No error expected")
 ...
 D CHKEQ^XTMUNIT("",^GMPL(125.12,"B",+NEWCAT,CATCMAX+1),"INCORRECT CROSS-REFERENCE")
