...
STARTUP ;
 ...
 S LOC=$P(^SC(0),U,3)
 S USER="1"
 F IN=1:1:10 D NEWLST^GMPLAPI1(.RET,"TestList"_IN)
 F IN=1:1:10 D NEWCAT^GMPLAPI1(.RET,"TestCateg"_IN)
 D ADDLIST(.LIST)
 ...
 D ASSUSR^GMPLAPI6(.RETURN,LSTLAST,DUZ)
 Q
 ;
SETUP ;
 Q
...
GETLSTS ;
 ...
 ;
 K RETURN N START
 D GETLSTS^GMPLAPI5(.RETURN,"TestList",.START,3)
 D CHKEQ^XTMUNIT("3"_U_"3"_U_"1"_U,RETURN(0),"Incorrect number of lists.")
 Q
 ;
GETCATS ;
 N RETURN,CNT,LIST
 D GETCATS^GMPLAPI5(.RETURN)
...
GETASUSR ;
 ...
 ;
 K RETURN
 D GETASUSR^GMPLAPI5(.RETURN,LSTLAST)
