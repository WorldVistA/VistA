...
API2NEW ;
 ...
 S %=$$NEW^GMPLAPI2(.GMPIFN,GMPDFN,GMPROV,.GMPFLD)
 ...
 D CHKEQ^XTMUNIT($G(^AUPNPROB(GMPIFN,1)),"304837^P^"_DUZ_"^1^1^^^"_+GMPCLIN_"^"_DT_"^0^0^0^0^A^^^^","Invalid data line 1. IFN: "_$G(GMPIFN,1))
...
API2DET ;
 ...
 D CHKEQ^XTMUNIT(RETURN(1.09),DT_"^"_DTF)
...
API2DETX ;
 ...
 D CHKEQ^XTMUNIT(RETURN("RECORDED"),DTF_"^TESTMASTER,USER")
...
API3NTE ;
 ...
 D CHKEQ^XTMUNIT(RETURN(1,1),"1^^TEST COMMENT^A^"_DT_"^1")
 D CHKEQ^XTMUNIT(RETURN(1,2),"2^^INACTIVATION NOTE^A^"_DT_"^1")
 D CHKEQ^XTMUNIT(RETURN(1,3),"3^^DELETE TESTS^A^"_DT_"^1")
 Q
 ;
API4LSTM ;
 N RETURN
