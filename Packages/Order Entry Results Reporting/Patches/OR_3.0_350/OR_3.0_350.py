...
class CustomInstaller(DefaultKIDSBuildInstaller):
  ...
  def __handleKIDSInstallQuestions__(self, connection, connection2=None):
    ...
    kidsMenuActionLst.append(("Send mail to", "", False))
    kidsMenuActionLst.append(("Select basket to send to", "", False))
    kidsMenuActionLst.append(("And Send to", "", False))
    while True:
  ...
  def runPreInstallationRoutine(self, connection, **kargs):
    connection.expect("Requested Start Time")
    connection.send("\r")