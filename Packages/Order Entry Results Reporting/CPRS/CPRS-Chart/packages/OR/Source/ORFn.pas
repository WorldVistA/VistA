...
type
  ...
  tSortDirArray = Array of tSortDir;
...
procedure InvertStringList(AList: TStrings);
procedure LimitStringLength(var AList: TStringList; MaxLength: Integer);
function MixedCase(const x: string; IgnoreSpecial: boolean = false): string;
procedure MixedCaseList(AList: TStrings);
procedure MixedCaseByPiece(AList: TStrings; ADelim: Char; PieceNum: Integer);
function VariableMixedCase(const x: string): string;
procedure VariableMixedCaseList(AList: TStrings);
procedure VariableMixedCaseByPiece(AList: TStrings; ADelim: Char; PieceNum: Integer);
function ExcludeFromMixed: TStringList;
procedure LoadExcludedFromMixed(LoadFromList: TStringList);
procedure SetExcludedDefault;
...
procedure SortByPieces(AList: TStringList; ADelim: Char; PieceNums: tSortArray;
  aSortDir: tSortDir = DIR_FRWRD); overload;
procedure SortByPieces(AList: TStringList; ADelim: Char; PieceNums: tSortArray;
...
{ Display functions }
procedure ForceInsideWorkArea(var Rect: TRect); overload;
procedure ForceInsideWorkArea(AForm: TForm); overload;
...
type
  IORKeepBounds = interface(IInterface)
  ['{10F15EED-68CB-4609-86E0-A937523B8C1C}']
...
begin
  if (aPos < 1) or (APos > Length(x)) then
    Result := #0
  else
    Result := x[APos];
end;
...
begin
 ...
 for I := 0 to fExcludeFromMixed.Count - 1 do
 begin
  //Need to be a space before
  LastPos := Pos(' ' + UpperCase(ExcludeFromMixed[i]), S);
  while LastPos <> 0 do
  begin
   //Are we at the end or if not then is the following not a letter
   if (LastPos + (Length(ExcludeFromMixed[i])) = Length(X)) or
   (not CharInSet(X[LastPos + 1 + Length(ExcludeFromMixed[i])], ['A'..'Z', 'a'..'z']))
    then begin
     Delete(Result, LastPos + 1, Length(ExcludeFromMixed[i]));
     Insert(UpperCase(ExcludeFromMixed[i]), Result, LastPos + 1);
   ...
   LastPos := Pos(' ' + UpperCase(ExcludeFromMixed[i]), S, LastPos + 1);
  end;
 end;
...
var
  ...
  aSorted: boolean;
begin
  aSorted := (AList is tStringList) and (tStringList(AList).Sorted);
  ...
  Finally
    if (AList is tStringList) then
      tStringList(AList).Sorted := aSorted;
  end;
...
begin
 result := x;
 if (Uppercase(x) = x) or (Lowercase(X) = x) then
  result := MixedCase(x);
end;
...
begin
  for i := 0 to (AList.Count - 1) do
  begin
    AList[i] := VariableMixedCase(AList[i]);
  end;
end;
...
begin
  ...
  begin
    x := AList[i];
    p := VariableMixedCase(Piece(x, ADelim, PieceNum));
    SetPiece(x, ADelim, PieceNum, p);
    AList[i] := x;
  end;
end;
...
procedure SortByPiece(AList: TStringList; ADelim: Char; PieceNum: Integer; aSortDir: tSortDir = DIR_FRWRD);
begin
  SortADelim := ADelim;
...
begin
  ...
  SortPieceNum := PieceNum;
  if aList is TSTringList then
    TSTringList(AList).CustomSort(SortByPiece2)
  else
    begin
      sl := TStringList.Create;
      try
        sl.Assign(aList);
        sl.CustomSort(SortByPiece2);
        FastAssign(sl, aList);
      finally
        sl.Free;
      end;
...
begin
  Result := 0;
  if Length(SortPieces) = 0 then
    exit;
  UseDirs := (length(SortDirs) > 0);
  if UseDirs and (Length(SortPieces) <> Length(SortDirs)) then
    exit;
  for i := 0 to Length(SortPieces) - 1 do
  begin
    SrtNum := SortPieces[i];
...
procedure SortByPieces(AList: TStringList; ADelim: Char;
  PieceNums: tSortArray; aSortDir: tSortDir = DIR_FRWRD);
begin
  SortADelim := ADelim;
  SortPieces := PieceNums;
  SortDir := aSortDir;
  SetLength(SortDirs, 0);
...
var
  ...
  begin
    ...
    else if UnitTestingActive then
      raise EUnitTestException.Create('Unsupported object type (' + obj.ClassName +
...
begin
  ...
  try
    str[0].SaveToStream(ms, TEncoding.Default);
    ms.Seek(0, soFromBeginning);
    str[1].LoadFromStream(ms, TEncoding.Default);
  finally
    ms.Free;
  end;
...
begin
  ...
  ms := TMemoryStream.Create;
  try
    str[1].SaveToStream(ms, TEncoding.Default);
    ms.Seek(0, soFromEnd);
    str[0].SaveToStream(ms, TEncoding.Default);
    ms.Seek(0, soFromBeginning);
    str[1].Clear;
...
procedure ShowMessage(const Text: string);
begin
  VAUtils.InfoDlg(Text, Application.Title, mtCustom, [mbOK], mbOK);
end;

function InfoBox(const Text, Caption: string; Flags: Word): Integer;
{ wrap the messagebox object in case we want to modify it later }
const
  BUTTON_MASK = $00000007;
  ICON_MASK = $000000F0;
  DEFAULT_BUTTON_MASK = $00000300;
...
begin
  ...
  HasDefault := True;
  dfltBtn := mbOK;
  DefaultButton := (Flags and DEFAULT_BUTTON_MASK);

  case (Flags and BUTTON_MASK) of
    MB_OK:
      begin
        Buttons := [mbOK];
        dfltBtn := mbOK;
    ...
    MB_OKCANCEL:
      begin
        ...
        if DefaultButton = MB_DEFBUTTON2 then
          dfltBtn := mbCancel;
      end;
    ...
    MB_YESNOCANCEL:
      begin
        ...
        if DefaultButton = MB_DEFBUTTON2 then
          dfltBtn := mbNo
        else if DefaultButton = MB_DEFBUTTON3 then
          dfltBtn := mbCancel;
      end;
    ...
    MB_ABORTRETRYIGNORE:
      begin
        ...
        if DefaultButton = MB_DEFBUTTON1 then
          dfltBtn := mbAbort
        else if DefaultButton = MB_DEFBUTTON3 then
          dfltBtn := mbIgnore;
      end;
  ...
  case (Flags and ICON_MASK) of
    MB_ICONERROR: DlgType := mtError;
  ...
  if HasDefault then
    Result := InfoDlg(Text, Caption, DlgType, Buttons, dfltBtn)
  else
    Result := InfoDlg(Text, Caption, DlgType, Buttons);
end;
...
begin
  if AControl is TForm then
    TForm(AControl).Font.Size := FontSize
  else
    with TFontControl(AControl) do
      if not ParentFont then
        Font.Size := FontSize;
  for i := 0 to AControl.ControlCount - 1 do
  begin
    if AControl.Controls[i] is TWinControl then
      ChangeAllFonts(AControl.Controls[i] as TWinControl, FontSize)
    else
      with TFontControl(AControl.Controls[i]) do
        if not ParentFont then
          Font.Size := FontSize;
  end;
end;
...
begin
  ...
  begin
    ...
    Paragraph.TabCount := MAX_TABS;
    for i := 0 to MAX_TABS - 1 do
      Paragraph.Tab[i] := PointsPerTabWidth * Succ(i);
  end;
end;
...
function WrappedTextHeightByFont(Canvas: TCanvas; NewFont: TFont;
  ItemText: string; var ARect: TRect): integer;
var
...
function ExcludeFromMixed: TStringList;
begin
  result := fExcludeFromMixed;
end;

procedure SetExcludedDefault;
begin
  fExcludeFromMixed.Add('II');
  fExcludeFromMixed.Add('III');
  fExcludeFromMixed.Add('IV');
  fExcludeFromMixed.Add('VI');
  fExcludeFromMixed.Add('VII');
  fExcludeFromMixed.Add('VIII');
  fExcludeFromMixed.Add('IX');
  fExcludeFromMixed.Add('-VA');
end;

procedure LoadExcludedFromMixed(LoadFromList: TStringList);
begin
 fExcludeFromMixed.Clear;
 fExcludeFromMixed.Assign(LoadFromList);
end;
