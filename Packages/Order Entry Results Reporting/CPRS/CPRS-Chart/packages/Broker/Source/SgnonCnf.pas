...
uses
  ...
  MFunStr, XWBut1,
  {Vcl}
  Vcl.Graphics, Vcl.Forms, Vcl.Controls, Vcl.Buttons, Vcl.StdCtrls,
...
type
  ...
  public
    ...
    property IntroFontStyles: String read FIntroFontStyles
      write SetIntroFontStyles;
...
begin
  ...
  if ReadRegDataDefault(HKCU, REG_SIGNON, 'IntroBackClr', '') <> '' then
    InitialValues.BackColor := StrToInt(ReadRegDataDefault(HKCU, REG_SIGNON,
      'IntroBackClr', ''));
...
begin
  ...
  try
    ...
    begin
      ...
      rgrWindowPosition.ItemIndex :=
        StrToInt(Piece(InitialValues.Position, U, 1));
...
begin
  ...
  if JPosition = 0 then
    ...
    FPosition := IntToStr(JPosition) + U + IntToStr(frmSignon.Top) + U +
      IntToStr(frmSignon.Left);
  strPosition := FPosition;

  if JSize = 0 then
    FSize := '0'
  else
    FSize := IntToStr(JSize) + U + IntToStr(frmSignon.Width) + U +
      IntToStr(frmSignon.Height);
  strSize := FSize;
