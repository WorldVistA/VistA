...
{ **************************************************
  ...
  identifier to work with ShellExecute(), TNetEncoding is used to format the
  URI
  3. Changed GetSelText function to virtual to allow changing type from string
  to AnsiString, this allows the text to be copied from the control
  in proper format to be passed into ShellExecute
...
uses
  ...
  {Vcl}
  Vcl.Controls, Vcl.Forms, Vcl.Menus, Vcl.Graphics, Vcl.StdCtrls, Vcl.ToolWin,
  Vcl.ImgList, Vcl.ExtCtrls, Vcl.ComCtrls, Vcl.Dialogs;

type
  TXWBCustomRichEdit = class;

  TAttributeType = (atSelected, atDefaultText);
  TConsistentAttribute = (caBold, caColor, caFace, caItalic, caSize,
    caStrikeOut, caUnderline, caProtected);
  ...
  TRichEditProtectChange = procedure(Sender: TObject; StartPos, EndPos: Integer;
  ...
  public
    ...
    function ConvertWriteStream(Stream: TStream; Buffer: PChar;
      BufSize: Integer): Integer; virtual;
  end;
  ...
  protected
    ...
    // p71 - made this function reintroduce to allow changing type from string to
    // to AnsiString, this allows the text to be copied from the control
    // in proper format to be passed to ShellExecute.  There may be a better way
    // to approach this.
    function GetSelText: AnsiString; reintroduce;
  ...
  public
    ...
    property DefAttributes: TXWBTextAttributes read FDefAttributes
      write SetDefAttributes;
    property SelAttributes: TXWBTextAttributes read FSelAttributes
...
uses
  {System}
  NetEncoding, AnsiStrings,
  {WinApi}
  ShellAPI,
  {Vcl}
  Vcl.Printers, Vcl.Consts, Vcl.ComStrs, Vcl.ActnList, Vcl.StdActns;

type
  PFontHandles = ^TFontHandles;

  TFontHandles = record
    OurFont, StockFont: Integer;
  end;

const
  SectionSizeArea = 8;
  RTFConversionFormat: TConversionFormat = (ConversionClass: TConversion;
    Extension: 'rtf'; Next: nil);
  TextConversionFormat: TConversionFormat = (ConversionClass: TConversion;
    Extension: 'txt'; Next: @RTFConversionFormat);
...
begin
  ...
  begin
    InitFormat(Format);
    SendMessage(RichEdit.Handle, EM_GETCHARFORMAT, WPARAM(FType = atSelected),
      LPARAM(@Format));
    with Format do
    begin
...
function TConversion.ConvertReadStream(Stream: TStream; Buffer: PChar;
  BufSize: Integer): Integer;
begin
  Result := Stream.Read(Buffer^, BufSize);
end;

function TConversion.ConvertWriteStream(Stream: TStream; Buffer: PChar;
  BufSize: Integer): Integer;
begin
...
begin
  ...
  if SendMessage(RichEdit.Handle, EM_LINELENGTH, SendMessage(RichEdit.Handle,
    EM_LINEINDEX, Result - 1, 0), 0) = 0 then
    Dec(Result);
end;
...
begin
  ...
  begin
    ...
    begin
      Selection.cpMax := Selection.cpMin + SendMessage(RichEdit.Handle,
        EM_LINELENGTH, Selection.cpMin, 0);
...
begin
  ...
  begin
    ...
    begin
      Selection.cpMin := SendMessage(RichEdit.Handle, EM_LINEINDEX,
        Index - 1, 0);
      if Selection.cpMin < 0 then
        Exit;
...
begin
  ...
  begin
    if Value then
      ...
      EventMask := SendMessage(Handle, EM_GETEVENTMASK, 0, 0) and
        not ENM_CHANGE;
...
function StreamSave(dwCookie: Longint; pbBuff: PByte; cb: Longint;
...
begin
  ...
  SendMessage(Handle, EM_AUTOURLDETECT, Ord(FURLDetect), 0);
  // Start the URL Detect
...
begin
  ...
  if Win32MajorVersion < 5 then
    Perform(WM_CONTEXTMENU, Handle,
      LPARAM(PointToSmallPoint(ClientToScreen(SmallPointToPoint
      (Message.Pos)))));
  inherited;
end;
...
function TXWBCustomRichEdit.GetSelTextBuf(Buffer: PChar;
  BufSize: Integer): Integer;
var
  S: AnsiString;
begin
  ...
  if BufSize < Length(S) then
    Result := BufSize;
  StrPLCopy(Buffer, String(S), Result);
end;

// function TXWBCustomRichEdit.GetSelText: string;
// p71 - changed return type from string to AnsiString to allow Unicode
// strings to be passed properly
function TXWBCustomRichEdit.GetSelText: AnsiString;
var
  Length: Integer;
begin
  SetLength(Result, GetSelLength + 1);
  Length := SendMessage(Handle, EM_GETSELTEXT, 0, Longint(Result));
...
begin
  with Message do
    case NMHdr^.code of
      ...
      // EN_LINK message being received, respond to it
      EN_LINK:
        begin
          Windows.SetCursor(Screen.Cursors[crHandPoint]);
          if PENLink(NMHdr)^.msg = WM_LBUTTONUP then
          begin
            // set the selection
            SendMessage(Handle, EM_EXSETSEL, 0, Longint(@PENLink(NMHdr)^.chrg));
            // send it to windows to open
            // old code: ShellExecute(handle, 'open', PWideChar(GetSelText), nil, nil, SW_SHOWNORMAL);
            // p71 - the line above fails under Unicode environment, using TNetEncoding
            // ensures the URI text copied from the control (AnsiString) is usable in ShellExecute
            ShellExecute(Handle, nil,
              PWideChar(TNetEncoding.URL.Decode(String(GetSelText))), nil, nil,
              SW_SHOWNORMAL);
          end;
        end;
    end;
...
begin
  ...
  if stWholeWord in Options then
    Flags := Flags or CommDlg.FR_WHOLEWORD; // p60
  if stMatchCase in Options then
    Flags := Flags or CommDlg.FR_MATCHCASE; // p60
...
class procedure TXWBCustomRichEdit.RegisterConversionFormat(const AExtension
