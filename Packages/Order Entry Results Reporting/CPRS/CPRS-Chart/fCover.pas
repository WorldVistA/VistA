...
type
  TfrmCover = class(TfrmPage)
    ...
    popMenuWomensHealth: TPopupMenu;
    popWHNewData: TMenuItem;
    popWHEnteredInError: TMenuItem;
    ...
    procedure popMenuWomensHealthPopup(Sender: TObject);
    procedure popWHEnteredInErrorClick(Sender: TObject);
    procedure popWHNewDataClick(Sender: TObject);
  private
...
uses
  ...
  iWVInterface; // TDrugs Patch OR*3*377 and WV*1*24 - DanP@SLC 11-20-2015
...
const
  ...
  TAG_VITL = 70;
  TAG_VSIT = 80;
  TAG_WVHT = 99; // TDrugs Patch OR*3*377 and WV*1*24 - DanP@SLC 11-20-2015
  RemID = '50';
...
begin
  ...
  if Length(FLoadingForDFN) > 0 then
    StopCoverSheet(FLoadingForDFN, uIPAddress, frmFrame.Handle); // *DFN*
  FLoadingForDFN := ''; // *DFN*
    ...
    begin
      ...
      for i := 0 to uCoverSheetList.Count - 1 do
        begin
          ...
          if AInvert = '1' then
            bInvert := True;
          if Length(ADatePiece) > 0 then
            iDatePiece := StrToInt(ADatePiece);
          if Length(aTextColor) > 0 then
            aTextColor := 'cl' + aTextColor;
          // Assign properties to components
          FCoverList.CVlbl(i).Caption := aName;
          FCoverList.CVlst(i).Caption := aName;
          if Length(aTabPos) > 0 then
            FCoverList.CVlst(i).TabPositions := aTabPos;
          if Length(aTextColor) > 0 then
            FCoverList.CVlst(i).Font.Color :=
              Get508CompliantColor(StringToColor(aTextColor));
          if Length(aPiece) > 0 then
            FCoverList.CVlst(i).Pieces := aPiece;
          ...
          if (AID <> RemID) then
            begin
              ...
              if (AID = '20') and ARTPatchInstalled then
                with FCoverList.CVlst(i) do
                  begin
              ...
              // TDrugs Patch OR*3*377 and WV*1*24 - DanP@SLC 11-20-2015
              if StrToIntDef(AID, -1) = TAG_WVHT then
                with FCoverList.CVlst(i) do
                  begin
                    PopupMenu := popMenuWomensHealth;
                    RightClickSelect := True;
                    popMenuWomensHealth.PopupComponent := FCoverList.CVlst(i);
                    Enabled := WVController.GetValidWVPatient(Patient.DFN);
                    WVController.AssignCoverSheetParams(FCoverList.CVlst(i), uCoverSheetList[i]);
                  end;
            end;
        ...
        if Components[i] is TORListBox then
          begin
            case Components[i].Tag of
              30:
                (Components[i] as TORListBox).Font.Size := NewFontSize;
            else
            end;
          end;
      end;
  ...
  if lb.ItemIndex <> -1 then
    begin
      ...
      case lb.Tag of
            ...
            begin
              if ARTPatchInstalled then
                begin
                  ...
                  if (frmARTAllergy <> nil) and frmARTAllergy.Showing then
                    frmARTAllergy.SetFocus;
                end
              else
                begin
                  ReportBox(DetailGeneric(lb.ItemIEN, lb.ItemID, ADetail), lb.DisplayText[lb.ItemIndex], True);
        ...
        TAG_POST:
          if lb.DisplayText[lb.ItemIndex] = 'Allergies' then
            begin
              ReportBox(DetailPosting('A'), lb.DisplayText[lb.ItemIndex], True);
            end
          else if CompareText(lb.ItemID, 'WH') = 0 then // TDrugs Patch OR*3*377 and WV*1*24 - DanP@SLC 11-20-2015
            begin
              ReportBox(DetailPosting('WH'), lb.DisplayText[lb.ItemIndex], True);
            end
          else if lb.ItemID <> '' then
            begin
              NotifyOtherApps(NAE_REPORT, 'TIU^' + lb.ItemID);
              ReportBox(DetailPosting(lb.ItemID), lb.DisplayText[lb.ItemIndex], True);
            end;
        TAG_MEDS:
          if (lb.ItemID <> '') and (lb.ItemID <> '0') then
            begin
        ...
        TAG_LABS:
          if (lb.ItemID <> '') and (Piece(lb.ItemID, ';', 1) <> '0') and
            (not ContainsAlpha(Piece(lb.ItemID, ';', 1))) then
            begin
              ReportBox(DetailGeneric(lb.ItemIEN, lb.ItemID, ADetail), lb.DisplayText[lb.ItemIndex], True);
            end;
        TAG_VITL:
          if lb.ItemID <> '' then
            begin
              ...
              if RemindersEvaluatingInBackground = True then
        ...
        TAG_WVHT: // TDrugs Patch OR*3*377 and WV*1*24 - DanP@SLC 11-20-2015
          if (lb.ItemID <> '') and (lb.ItemID <> '0') then
            begin
              ReportBox(DetailGeneric(lb.ItemIEN, lb.ItemID, ADetail), lb.DisplayText[lb.ItemIndex], True);
            end;
  ...
  if InteractiveRemindersActive then
    begin
      ...
      try
        ...
        if (iProb > -1) and (ProbSL.Count > 0) then
          FastAssign(ProbSL, (Components[iProb] as TORListBox).Items);
        if (iPost > -1) and (PostSL.Count > 0) then
          FastAssign(PostSL, (Components[iPost] as TORListBox).Items);
        if (iMeds > -1) and (MedsSL.Count > 0) then
          FastAssign(MedsSL, (Components[iMeds] as TORListBox).Items);
        if (iLabs > -1) and (LabsSL.Count > 0) then
          FastAssign(LabsSL, (Components[iLabs] as TORListBox).Items);
        if (iVit > -1) and (VitSL.Count > 0) then
          FastAssign(VitSL, (Components[iVit] as TORListBox).Items);
        if (iVisit > -1) and (VisitSL.Count > 0) then
          FastAssign(VisitSL, (Components[iVisit] as TORListBox).Items);
    ...
    begin
      ...
      if (iMeds > -1) and (MedsSL.Count > 0) then
        FastAssign(MedsSL, (Components[iMeds] as TORListBox).Items);
      if (iRem > -1) and (RemSL.Count > 0) then
        FastAssign(RemSL, (Components[iRem] as TORListBox).Items);
      if (iLabs > -1) and (LabsSL.Count > 0) then
        FastAssign(LabsSL, (Components[iLabs] as TORListBox).Items);
      if (iVit > -1) and (VitSL.Count > 0) then
        FastAssign(VitSL, (Components[iVit] as TORListBox).Items);
                ...
                begin
                  case Components[i].Tag of
                    20:
                      UpdateAllergiesList;
                    30:
                      ListPostings((Components[i] as TORListBox).Items);
                  end;
                end;
            end;
      end;
...
// TDrugs Patch OR*3*377 and WV*1*24 - DanP@SLC 11-20-2015
procedure TfrmCover.popMenuWomensHealthPopup(Sender: TObject);
var
  aListBox: TORListBox;
  aCaption: string;
begin
  aListBox := (popMenuWomensHealth.PopupComponent as TORListBox);
  if aListBox.ItemID <> '' then
    begin
      if WVController.GetRecordIDType(aListBox.ItemID, aCaption) then
        begin
          popWHEnteredInError.Enabled := True;
          popWHEnteredInError.Caption := Format('Mark item ''%s'' as entered in error ...', [aCaption]);
        end
      else
        begin
          popWHEnteredInError.Enabled := FALSE;
          popWHEnteredInError.Caption := aCaption;
        end
    end
  else
    begin
      popWHEnteredInError.Enabled := FALSE;
      popWHEnteredInError.Caption := 'Nothing to mark as entered in error';
    end;
end;

// TDrugs Patch OR*3*377 and WV*1*24 - DanP@SLC 11-20-2015
procedure TfrmCover.popWHEnteredInErrorClick(Sender: TObject);
var
  aListBox: TORListBox;
begin
  aListBox := (popMenuWomensHealth.PopupComponent as TORListBox);
  if aListBox.ItemID <> '' then
    if WVController.MarkAsEnteredInError(aListBox.ItemID) then // Do the redraw if it was marked as Entered in Error
      with WVController.CoverSheetParams do
        begin
          LoadList(Status, CoverSheetObject, MainRPC, TitleCase, Invert, DatePiece, DateFormat, Param1, ID, DetailRPC, True);
          frmFrame.RefreshCWAD;
        end;
end;

// TDrugs Patch OR*3*377 and WV*1*24 - DanP@SLC 11-20-2015
procedure TfrmCover.popWHNewDataClick(Sender: TObject);
var
  aWVPatient: IWVPatient;
begin
  if WVController.GetWVPatient(Patient.DFN, aWVPatient) then
    if WVController.EditPregLacData(aWVPatient) then // Do the redraw if it was updated
      with WVController.CoverSheetParams do
        begin
          LoadList(Status, CoverSheetObject, MainRPC, TitleCase, Invert, DatePiece, DateFormat, Param1, ID, DetailRPC, True);
          frmFrame.RefreshCWAD;
...
begin
  ...
  // Assign properties to components
  if Length(aTabPos) > 0 then
    (popMenuAllergies.PopupComponent as TORListBox).TabPositions := aTabPos;
  if Length(aTextColor) > 0 then
    (popMenuAllergies.PopupComponent as TORListBox).Font.Color :=
      Get508CompliantColor(StringToColor(aTextColor));
  if Length(aPiece) > 0 then
    (popMenuAllergies.PopupComponent as TORListBox).Pieces := aPiece;
...
if assigned(fCover.VAAFlag) then
  fCover.VAAFlag.Free; // VAA
