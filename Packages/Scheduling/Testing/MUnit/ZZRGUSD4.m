...
MAKEUS ;
 ...
 D CHKEQ^%ut(RE_U_$P(RE(0),U),"0^INVPARAM","Expected: INVPARAM TYPE")
 D CHKEQ^%ut($P(RE(0),U,2)["TYPE",1,"Expected: INVPARAM TYPE")
 ;
 S %=$$MAKEUS^SDMAPI2(.RE,DFN,SC,,TYPE)
 D CHKEQ^%ut(RE,1,"Unexpected error: ")
 S SC0=+DFN_"^"_+LEN_"^^^^"_DUZ_"^"_DT
 D CHKEQ^%ut(^SC(+SC,"S",+RE(1),1,1,0),SC0,"Invalid clinic appointment - 0 node")
 S DPT0=+SC_"^^^^^^4^^^^^^^^^"_+TYPE_"^^"_DUZ_"^"_DT_"^^^^^0^W^0"
 D CHKEQ^%ut(^DPT(+DFN,"S",+RE(1),0),DPT0,"Invalid patient appointment - 0 node")
 ;future appt
 ...
 D CHKEQ^%ut(RETURN,0,"Expected error: APTCINV")
 D CHKEQ^%ut($P(RETURN(0),U),"APTCINV","Expected error: APTCINV")
 K ^SC(+SC,"I")
 Q
MAKECO ;
 N RETURN,%,COD
 ...
 S %=$$MAKEUS^SDMAPI2(.RETURN,DFN,SC,SD0,TYPE,,.CIO)
 D CHKEQ^%ut(RETURN,1,"Unxpected error: "_$G(RETURN(0)))
 D CHKEQ^%ut(+$G(RETURN("COD")),COD,"Incorrect check out date")
...
MAKECI ;
 ...
 S %=$$MAKEUS^SDMAPI2(.RETURN,DFN,SC,SD0,TYPE,,.CIO)
 D CHKEQ^%ut(RETURN,1,"Unxpected error: "_$G(RETURN(0)))
 D CHKEQ^%ut($G(RETURN("CI")),"","Future Appt cannot be checked in.")
 ;appt checked in
 S SDPAST=+$E($$NOW^XLFDT(),1,12)
 S %=$$MAKEUS^SDMAPI2(.RETURN,DFN,SC,SDPAST,TYPE,,.CIO)
 D CHKEQ^%ut(RETURN,1,"Unxpected error: "_$G(RETURN(0)))
 D CHKEQ^%ut(+$G(RETURN("CI")),+SDPAST,"Incorrect check in date")
 ;past appointment cannot be checked in
 S SDPAST=$E($$FMADD^XLFDT($$NOW^XLFDT(),-1),1,10)
 S %=$$MAKEUS^SDMAPI2(.RETURN,DFN,SC,SDPAST,TYPE,,.CIO)
 D CHKEQ^%ut(RETURN,1,"Unxpected error: "_$G(RETURN(0)))
 D CHKEQ^%ut($G(RETURN("CI")),"","Past Appt, cannot be checked in.")
 K ^XUSEC("SDOB",DUZ),^XUSEC("SDMOB",DUZ)
 Q
 ;
CHKTYP ; Check appt type
 ...
 D CHKEQ^%ut(RETURN,0,"Unxpected error: "_$G(RETURN(0)))
 D CHKEQ^%ut(RETURN(0),"INVPARAM^Invalid parameter value - TYPE^1","Expected error: INVPARAM")
 ; Undefined type
 S SD0=$$NOW^XLFDT(),SD0=$E(SD0,1,12)+0.0001
 S %=$$MAKEUS^SDMAPI2(.RETURN,DFN,SC,SD0,$P(^SD(409.1,0),U,3)+1,,)
 D CHKEQ^%ut(RETURN,0,"Unxpected error: "_$G(RETURN(0)))
 D CHKEQ^%ut(RETURN(0),"TYPNFND^Appointment type not found.^1","Expected error: TYPNFND")
 ; Out of sync service conected
 S SD0=$$NOW^XLFDT(),SD0=$E(SD0,1,12)+0.0001
 S %=$$MAKEUS^SDMAPI2(.RETURN,DFN,SC,SD0,11,,)
 D CHKEQ^%ut(RETURN,0,"Unxpected error: "_$G(RETURN(0)))
 D CHKEQ^%ut(RETURN(0),"TYPINVSC^The 'SC Percent','Service Connected' and 'Primary Eligibility Codes' are OUT OF^1","Expected error: TYPINVSC")
 ; Out of sync service conected
 S SD0=$$NOW^XLFDT(),SD0=$E(SD0,1,12)+0.0001
 S %=$$MAKEUS^SDMAPI2(.RETURN,DFN,SC,SD0,4,,)
 D CHKEQ^%ut(RETURN,0,"Unxpected error: "_$G(RETURN(0)))
 D CHKEQ^%ut(RETURN(0),"TYPINVD^Patient must have the eligibility code EMPLOYEE, COLLATERAL or SHARING AGREEMENT^1","Expected error: TYPINVD")
 Q
CHKSTYP ; Check appt subtype
 ...
 D CHKEQ^%ut(RETURN,0,"Unxpected error: "_$G(RETURN(0)))
 D CHKEQ^%ut(RETURN(0),"STYPNFND^Appointment subtype not found or inactive.^1","Expected error: STYPNFND")
 ; Unscheduled appointment subtype
 S SDD=DT_".08",SDD=SDD_U_$$FMTE^XLFDT(SDD)
 S %=$$MAKEUS^SDMAPI2(.RETURN,DFN,SC,SDD,TYPE,STYP)
 D CHKEQ^%ut(RETURN,1,"Unexpected error: ")
 S SC0=+DFN_"^"_+LEN_"^^^^"_DUZ_"^"_DT
 D CHKEQ^%ut(^SC(+SC,"S",+SDD,1,1,0),SC0,"Invalid clinic appointment - 0 node")
 S DPT0=+SC_"^^^^^^4^^^^^^^^^"_+TYPE_"^^"_DUZ_"^"_DT_"^^^^^"_+STYP_"^W^0"
 D CHKEQ^%ut(^DPT(+DFN,"S",+SDD,0),DPT0,"Invalid patient appointment - 0 node")
 ; Invalid sub type
...
LSTASTYP ;Check appt subtype
 ...
 D CHKEQ^%ut(RETURN(0),"2^*^0^","Invalid 0 node")
 D CHKEQ^%ut(RETURN(1,"ID"),1,"Invalid appt subtype ID")
 D CHKEQ^%ut(RETURN(1,"NAME"),"Sharing 1","Invalid appt subtype NAME")
 D CHKEQ^%ut(RETURN(1,"STATUS"),"YES","Invalid appt subtype NAME")
 D CHKEQ^%ut(RETURN(2,"ID"),2,"Invalid appt subtype ID")
 D CHKEQ^%ut(RETURN(2,"NAME"),"Sharing 2","Invalid appt subtype NAME")
 D CHKEQ^%ut(RETURN(2,"STATUS"),"YES","Invalid appt subtype NAME")
 Q
MAKENA ; Request type
 N RE,%
 ...
 D CHKEQ^%ut(RE_U_$P(RE(0),U),"0^SRTNFND","Expected error: SRTNFND")
...
MAKELAB ; LAB, EKG or X-RAY
 ...
 D CHKEQ^%ut(RE_U_$P($G(RE(0)),U),"0^INVPARAM","Expected error: INVPARAM")
 ; Invalid LAB
 S %=$$MAKE^SDMAPI2(.RE,DFN,SC,SD1,TYPE,,LEN,NXT,RSN,,SD1,,,,,1)
 D CHKEQ^%ut(RE_U_$P($G(RE(0)),U),"0^TSTAHAPT","Expected error: TSTAHAPT")
 ; Invalid XRAY
 ...
 D CHKEQ^%ut(RE_U_$P($G(RE(0)),U),"0^CNSNFND","Expected error: CNSNFND")
 ; Valid appt
 S ^GMR(123,1,0)=""
 S %=$$MAKE^SDMAPI2(.RE,DFN,SC,SD1,TYPE,,LEN,NXT,RSN,,LAB,LAB,LAB,,1,1)
 D CHKEQ^%ut(RE,1,"Unexpected error: "_$G(RE(0)))
