ZZUTNEXTAPPT ;kitware/JNL - code for a unit test NEXTAPPT tag;07/10/12  11:15
 ...
 TSTART
 I $T(EN^%ut)'="" D EN^%ut("ZZUTNEXTAPPT")
 TROLLBACK
 Q
 ;
...
CHKINVALDCOMB(APPTSLST,PSTATLIST) ; check the combination of patient status and appt status
 ...
 Q
CHKERRCODE(ERRCODE) ;
 D CHKTF^%ut($D(^TMP($J,"SDAMA201","NEXTAPPT","ERROR",ERRCODE))'=0,"Expecting Error Code is "_ERRCODE)
 Q
 ;
CHKPATAPPT ; unit test case to check the appointment date
 ...
 D CHKEQ^%ut(RCODE,1,"Should only have one appointment")
...
CHKAPPTFLDDETAIL(NODEID,FLDID) ;
 D CHKEQ^%ut(^TMP($J,"SDAMA201","NEXTAPPT",NODEID,FLDID),$TR($P(NEXTAPPTRST1,U,FLDID),";",U),"FIELD: "_FLDID_" RESULT MISMATCH!")
 Q
XTROU ;
 ;;;SDAMA201
