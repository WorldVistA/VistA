...
MAKE ;
 ...
 D CHKEQ^%ut(RE_U_$P(RE(0),U),"0^INVPARAM","Expected: INVPARAM LEN")
 D CHKEQ^%ut($P(RE(0),U,2)["LEN",1,"Expected: INVPARAM LEN")
 ...
 D CHKEQ^%ut(^SC(+SC2,"S",+SD,1,1,0),SC0,"Invalid clinic appointment - 0 node")
...
NOSHOW ;
 ...
 . D CHKEQ^%ut(RETURN,0,"Expected error: APTCNPE")
 . D CHKEQ^%ut($P(RETURN(0),U),"APTCNPE","Expected error: APTCNPE")
 S $P(^GMR(123,CONS,0),U,12)=13
 S %=$$GETAPCNS^SDCAPI1(.RETURN,DFN,SCODE)
 D CHKEQ^%ut(RETURN,1,"Unexpected error: "_$G(RETURN(0)))
 S %=$$NOSHOW^SDMAPI2(.RETURN,DFN,SC,SD),NOW=$$NOW^XLFDT()
 D CHKEQ^%ut(RETURN,0,"Expected error: APTNSAL")
 D CHKEQ^%ut($P(RETURN(0),U),"APTNSAL","Expected error: APTNSAL")
 S %=$$NOSHOW^SDMAPI2(.RETURN,DFN,SC,SD,1),NOW=$$NOW^XLFDT()
 D CHKEQ^%ut(RETURN,1,"Unxpected error: "_$G(RETURN(0)))
 D CHKEQ^%ut($P(^DPT(+DFN,"S",+SD,0),U,2),"","Unxpected error: "_$G(RETURN(0)))
 ...
 D CHKEQ^%ut(RE_U_$P(RE(0),U),"0^APTNSCE","Expected: APTNSCE")
 S $P(^SC(+SC,0),U,17)="Y"
 S %=$$NOSHOW^SDMAPI2(.RE,DFN,SC,$$FMADD^XLFDT(SD,1))
...
CANCEL ;
 ...
 D CHKEQ^%ut(RETURN,0,"Expected error: RSNNFND")
 D CHKEQ^%ut($P(RETURN(0),U),"RSNNFND","Expected error: RSNNFND")
 ...
 D CHKEQ^%ut(RE_U_$P(RE(0),U),"0^INVPARAM","Expected: INVPARAM TYP")
 D CHKEQ^%ut($P(RE(0),U,2)["TYP",1,"Expected: INVPARAM TYP")
 ;undefined SD
 ...
 D CHKEQ^%ut(RE_U_$P(RE(0),U),"0^INVPARAM","Expected: INVPARAM RSN")
 D CHKEQ^%ut($P(RE(0),U,2)["RSN",1,"Expected: INVPARAM RSN")
 ;reason not found
 S %=$$CANCEL^SDMAPI2(.RE,DFN,SC,SD,"C",$P(^SD(409.2,0),U,3)+1,)
 D CHKEQ^%ut(RE_U_$P(RE(0),U),"0^RSNNFND","Expected: RSNNFND")
 ;appt not found
 S %=$$CANCEL^SDMAPI2(.RE,DFN,SC,2,"C",CRSN,)
 D CHKEQ^%ut(RE_U_$P(RE(0),U),"0^APTNFND","Expected: APTNFND")
 ; Already cancelled
 S %=$$CANCEL^SDMAPI2(.RETURN,DFN,SC,SD,"PC",CRSN,"Cancellation test remarks")
 D CHKEQ^%ut(RETURN,0,"Expected error: APTCAND")
 D CHKEQ^%ut($P(RETURN(0),U),"APTCAND","Expected error: APTCAND")
 S $P(^DPT(+DFN,"S",+SD,0),U,2)=""
 ; Checked out
 S $P(^SC(+SC,"S",+SD,1,1,"C"),U,3)=+SD
 S %=$$CANCEL^SDMAPI2(.RETURN,DFN,SC,SD,"PC",CRSN,"Cancellation test remarks")
 D CHKEQ^%ut(RETURN,0,"Expected error: APTCCHO")
 D CHKEQ^%ut($P(RETURN(0),U),"APTCCHO","Expected error: APTCCHO")
 S $P(^SC(+SC,"S",+SD,1,1,"C"),U,3)=""
 ; User rights
 S ^SC(+SC,"SDPROT")="Y"
 S %=$$CANCEL^SDMAPI2(.RETURN,DFN,SC,SD,"PC",CRSN,"Cancellation test remarks")
 D CHKEQ^%ut(RETURN,0,"Expected error: APTCRGT")
 D CHKEQ^%ut($P(RETURN(0),U),"APTCRGT","Expected error: APTCRGT")
 K ^SC(+SC,"SDPROT")
 ;
 S NOW=$$NOW^XLFDT(),%=$$CANCEL^SDMAPI2(.RETURN,DFN,SC,SD,"PC",CRSN)
 S SC0=+DFN_"^"_+LEN_"^^"_+CRSN_"^^"_DUZ_"^"_DT_"^^^"
 D CHKEQ^%ut($G(^SC(+SC,"S",+SD,1,1)),"","Invalid clinic appointment - 0 node")
 S DPT0=+SC_"^PC^^^^^3^^^^^"_DUZ_"^^"_+$J($$NOW^XLFDT(),4,2)_"^"_+CRSN_"^"_+TYPE_"^^"_DUZ_"^"_DT_"^^^^^0^"_NXT_"^3"
 S DPT1=^DPT(+DFN,"S",+SD,0)
 S $P(DPT1,"^",14)=+$P(DPT1,"^",14)
 D CHKEQ^%ut(DPT1,DPT0,"Invalid patient appointment - 0 node")
 ;
