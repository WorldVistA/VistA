...
CHKRCODE   ; Unit test to test the return code of $$SDAPI^SDAMA301
 ...
 D CHKTF^%ut($D(^TMP($J,"SDAMA301",115))'=0,"Expecting Error Code is 115")
...
CHKPATAPPT ; unit test case to check the appointment date
 ...
 S SDARRAY("FLDS")="ALL"
 S RCODE=$$SDAPI^SDAMA301(.SDARRAY)
 D CHKEQ^%ut(RCODE,10,"Expected rcode 10, real: "_RCODE)
 D CHKTF^%ut($D(^TMP($J,"SDAMA301"))'=0,"^TMP global should not be empty")
 D CHKTF^%ut($D(^TMP($J,"SDAMA301",VISN0PAT1,VISN0CLI1,VISN0APPT1P1C1))'=0,"Should have APPT at "_VISN0APPT1P1C1)
 ...
 S RCODE=0
 S RCODE=$$SDAPI^SDAMA301(.SDARRAY)
 D CHKTF^%ut(RCODE=1,"Should have only one appointment")
 D CHKTF^%ut($D(^TMP($J,"SDAMA301"))'=0,"^TMP global should not be empty")
 D CHKTF^%ut($D(^TMP($J,"SDAMA301",VISN0PAT1,VISN0CLI1,VISN0APPT1P1C1))'=0,"Should have APPT at "_VISN0APPT1P1C1)
 S APPNODE=^TMP($J,"SDAMA301",VISN0PAT1,VISN0CLI1,VISN0APPT1P1C1)
 S APPNODE1=^TMP($J,"SDAMA301",VISN0PAT1,VISN0CLI1,VISN0APPT1P1C1,0) ; extra information
 ...
 S SDARRAY(3)="NS"
 S RCODE=$$SDAPI^SDAMA301(.SDARRAY)
 D CHKTF^%ut(RCODE=0,"Should not have any appointment")
 D CHKTF^%ut($D(^TMP($J,"SDAMA301"))=0,"^TMP global should be empty")
 I RCODE K ^TMP($J,"SDAMA301")
 ...
 D CHKTF^%ut($D(^TMP($J,"SDAMA301"))>0,"^TMP global should NOT be empty")
 D CHKTF^%ut($D(^TMP($J,"SDAMA301",VISN0PAT1,VISN0APPT1P1C1))>0,"Should have found the appointment")
 I RCODE K ^TMP($J,"SDAMA301")
 Q
 ;
CHKVISN0APT1(APPNODE) ;
 D CHKEQ^%ut(VISN0APPT1P1C1,$P(APPNODE,U,1),"Should be from the same time as: "_VISN0APPT1P1C1)
 D CHKEQ^%ut(VISN0CLI1,$P($P(APPNODE,U,2),";",1),"Should be from the clinic id: "_VISN0CLI1)
 D CHKEQ^%ut("R",$P($P(APPNODE,U,3),";",1),"Status should be: R")
 D CHKEQ^%ut(VISN0PAT1,$P($P(APPNODE,U,4),";",1),"Patient IEN should be: "_VISN0PAT1)
 D CHKEQ^%ut(15,$P(APPNODE,U,5),"Appointment Length should be: 15 minutes")
 D CHKEQ^%ut("REGULAR",$P($P(APPNODE,U,10),";",2),"Appointment should be regular")
 D CHKEQ^%ut("141;301",$P(APPNODE,U,13),"Primary Stop Code IEN and code should be 141 and 301")
 D CHKEQ^%ut("3030409",$P(APPNODE,U,16),"Appointment was made at 3030409")
 ;Check if X-Ray films are required
 D CHKEQ^%ut("Y",$P(APPNODE,U,23),"Need to take X-Ray films")
 Q
 ;
CHKVISN0APT1EXTRA(APPNODE) ;
 D CHKEQ^%ut(11729,$P($P(APPNODE,U,1),";",1),"Date Entry Clerk' DUZ should be 11729")
 Q
 ;
HASAPPT(PATIEN) ;
 N SDARRAY,RCODE
