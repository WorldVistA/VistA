...
class CustomInstaller(DefaultKIDSBuildInstaller):
  def __init__(self, kidsFile, kidsInstallName,
    ...
    assert kidsInstallName == "GMRC*3.0*123" and int(seqNo) == 121
  ...
  def runPreInstallationRoutine(self, connection, **kargs):
    try:
      index = connection.expect(["PLEASE ENTER THE HEALTHCONNECT SERVER IP ADDRESS", "Updating KIDS files"], 5)
      if index == 1:
         return True
      connection.send("127.0.0.1")
      connection.expect("PLEASE ENTER THE HEALTHCONNECT SERVER PORT NUMBER")
      connection.send("99999")
